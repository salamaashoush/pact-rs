...
(module capability-examples 'admin-keyset
  ...
  (defcap TIME_LOCKED_ADMIN (unlock-time:time)
    ...
    (enforce (>= (chain-data 'time) unlock-time) "Time lock active")
    (enforce-keyset 'admin-keyset))
  ...
  (defcap MULTI_SIG ()
    ...
    (let ((signers (length (env-keys))))
      (enforce (>= signers 2) "Requires at least 2 signatures")))
  
  (defun multi-sig-function ()
    @doc "Function requiring multiple signatures"
    (with-capability (MULTI_SIG)
  ...
  (defcap RATE_LIMITED (user:string)
  ...
  (defun rate-limited-action (user:string)
    ...
    (with-capability (RATE_LIMITED user)
