...
-- Helper to convert Maybe to InL/InR response  
maybeToNull :: Maybe a -> a |? Null
maybeToNull Nothing = InR Null
maybeToNull (Just a) = InL a
...
termAt p term
  | p `inside` view termInfo term = 
      -- First try to find a more specific nested term
      let nestedMatch = case term of
            Lam _ b _ -> termAt p b
            App tm1 tm2 _ ->
              termAt p tm1 <|> getAlt (foldMap (Alt . termAt p) tm2)
            Let _ tm1 tm2 _ -> termAt p tm1 <|> termAt p tm2
            ...
            BuiltinForm op' _ ->
              case op' of
                ...
                CError a -> termAt p a
                CPure a -> termAt p a
            ListLit tms _ -> getAlt (foldMap (Alt . termAt p) tms)
            Nullary tm _ -> termAt p tm
            ObjectLit l _ -> getAlt (foldMap (\(_, tm) -> Alt (termAt p tm)) l)
            _ -> Nothing
      -- If we found a nested match, return it; otherwise return this term
      in nestedMatch <|> Just term
  | otherwise = Nothing
