...
use pact_parser::{ParsedTopLevel, ParsedExpr, ParsedName as AstParsedName, LetForm, Binder, MArg};
...
/// Lower a literal
fn lower_literal(lit: &pact_parser::Literal) -> Result<Literal, PactError<SpanInfo>> {
    match lit {
        pact_parser::Literal::LString(s) => Ok(Literal::LString(CompactString::from(s.as_str()))),
        pact_parser::Literal::LInteger(n) => Ok(Literal::LInteger(*n)),
        pact_parser::Literal::LDecimal(d) => {
            // Convert from pact_parser::Decimal to pact_ir::Decimal
            Ok(Literal::LDecimal(Decimal {
                precision: d.precision,
                mantissa: d.mantissa,
            }))
        }
        pact_parser::Literal::LBool(b) => Ok(Literal::LBool(*b)),
        pact_parser::Literal::LUnit => Ok(Literal::LUnit),
    }
}
...
/// Lower a type
fn lower_type(ty: &pact_parser::Type) -> Result<Type, PactError<SpanInfo>> {
    use pact_parser::Type as ParsedType;
    use pact_parser::PrimType;
