...
impl SemanticTokenProvider {
    ...
    /// Add semantic tokens from AST analysis
    fn add_semantic_tokens(&self, tokens: &mut Vec<SemanticToken>, program: &pact_syntax::Program, text: &str) -> Result<(), String> {
        let lines: Vec<&str> = text.lines().collect();
    ...
    fn add_type_based_tokens(&self, tokens: &mut Vec<SemanticToken>, program: &pact_syntax::Program, text: &str) -> Result<(), String> {
