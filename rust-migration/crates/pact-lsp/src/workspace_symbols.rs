...
use pact_syntax::{parse, Declaration, TopLevel, Expr};
...
impl WorkspaceSymbolProvider {
    ...
    fn create_function_detail(&self, name: &str, params: &[pact_syntax::Parameter], return_type: &Option<pact_syntax::Type>) -> String {
    ...
    fn create_capability_detail(&self, name: &str, params: &[pact_syntax::Parameter]) -> String {
        let param_list = params.iter()
            .map(|p| {
                if let Some(ty) = &p.ty {
                    format!("{}: {}", p.name, self.type_to_string(ty))
    ...
    fn create_constant_detail(&self, name: &str, type_ann: &Option<pact_syntax::Type>) -> String {
    ...
    fn create_schema_detail(&self, name: &str, fields: &[pact_syntax::Field]) -> String {
    ...
    fn create_pact_detail(&self, name: &str, params: &[pact_syntax::Parameter], step_count: usize) -> String {
        let param_list = params.iter()
            .map(|p| {
                if let Some(ty) = &p.ty {
                    format!("{}: {}", p.name, self.type_to_string(ty))
    ...
    fn type_to_string(&self, ty: &pact_syntax::Type) -> String {
        match ty {
            pact_syntax::Type::String => "string".to_string(),
            pact_syntax::Type::Integer => "integer".to_string(),
            pact_syntax::Type::Decimal => "decimal".to_string(),
            pact_syntax::Type::Bool => "bool".to_string(),
            pact_syntax::Type::Time => "time".to_string(),
            pact_syntax::Type::Keyset => "keyset".to_string(),
            pact_syntax::Type::Guard => "guard".to_string(),
            pact_syntax::Type::List(inner) => format!("[{}]", self.type_to_string(inner)),
            pact_syntax::Type::Object(obj_type) => {
                match obj_type {
                    pact_syntax::ObjectType::Schema(name) => name.clone(),
                    pact_syntax::ObjectType::Fields(_) => "object".to_string(),
                }
            },
            pact_syntax::Type::ModuleRef(name) => format!("module<{}>", name),
            pact_syntax::Type::TypeVar(name) => name.clone(),
        }
    }
}