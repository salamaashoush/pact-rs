...
impl PactDb for SqliteDb {
        ...
        self.with_connection(|conn| {
            ...
            for row in rows {
                ...
                if let Some(underscore_pos) = table_key.find('_') {
                    ...
                    let module = pact_core::names::ModuleName::simple(module_part.to_string());
    ...
    fn read_module(&self, module_name: &pact_core::names::ModuleName) -> DbResult<Option<pact_ir::CoreModuleData>> {
    ...
    fn module_exists(&self, module_name: &pact_core::names::ModuleName) -> DbResult<bool> {
        Ok(self.read_raw(&Domain::Modules, &module_name.render())?.is_some())
    }
    
    fn list_modules(&self) -> DbResult<Vec<pact_core::names::ModuleName>> {
        let keys = self.keys_raw(&Domain::Modules)?;
        let mut modules = Vec::new();
        
        for key in keys {
            modules.push(pact_core::names::ModuleName::parse(&key));
        }
        
        Ok(modules)
    }
    
    fn write_module_source(&self, module_name: &pact_core::names::ModuleName, hash: &pact_ir::ModuleHash, source: &str) -> DbResult<()> {
        let key = format!("{}#{}", module_name.render(), hash.0);
        self.write_raw(WriteType::Write, &Domain::ModuleSource, &key, source.as_bytes())
    }
    
    fn read_module_source(&self, module_name: &pact_core::names::ModuleName, hash: &pact_ir::ModuleHash) -> DbResult<Option<String>> {
        let key = format!("{}#{}", module_name.render(), hash.0);
