...
pub fn register_module_builtins(builtin_env: &mut BuiltinEnv) -> Result<(), pact_core::errors::PactErrorI> {
                ...
                match arg0 {
                    CEKValue::VPactValue(PactValue::String(module_name_str)) => {
                        ...
                        if env.flags.disallow_local_only_builtins {
                            return EvalM::pure_value(EvalResult::EvalError(
                                pact_core::errors::PactError::PEExecutionError(
                                    pact_core::errors::EvalError::InvalidExecutionContext(format!("describe-module: Only allowed in /local execution")),
                                    vec![],
                                    SpanInfo::empty()
                                )
                            ));
                        ...
                        match read_module_from_storage(&module_name_obj, &*env.pact_db) {
                            ...
                            Ok(None) => {
                                ...
                                return EvalM::pure_value(EvalResult::EvalError(
                                    pact_core::errors::PactError::PEExecutionError(
                                        pact_core::errors::EvalError::InvalidExecutionContext(format!("Module not found: {}", module_name_str)),
                                        vec![],
                                        SpanInfo::empty()
                                    )
                                ));
                        ...
                        let result = CEKValue::VPactValue(
                            PactValue::Object(pact_core::values::Object::from_hashmap(module_obj))
                        );
        ...
        charge_gas("list-modules", MilliGas(50))
            .bind(move |_| {
                ...
                if env.flags.disallow_local_only_builtins {
                    return EvalM::pure_value(EvalResult::EvalError(
                        pact_core::errors::PactError::PEExecutionError(
                            pact_core::errors::EvalError::InvalidExecutionContext(format!("list-modules: Only allowed in /local execution")),
                            vec![],
                            SpanInfo::empty()
                        )
                    ));
            ...
            .bind(move |_| {
                match arg0 {
                    CEKValue::VPactValue(PactValue::String(module_name)) => {
                        ...
                        let guard = pact_core::values::Guard::module(module_name, None);
...
fn define_namespace_implementation() -> NativeFunction {
    Box::new(|info, builtin, cont, handler, env, args| {
        ...
        if env.call_stack_depth() > 1 {
            return EvalM::pure_value(EvalResult::EvalError(
                pact_core::errors::PactError::PEExecutionError(
                    pact_core::errors::EvalError::InvalidExecutionContext(format!("define-namespace: Only allowed at top level")),
                    vec![],
                    SpanInfo::empty()
                )
            ));
            ...
            .bind(move |_| {
                match arg0 {
                    CEKValue::VPactValue(PactValue::String(namespace)) => {
                        ...
                        let result = CEKValue::VPactValue(
                            PactValue::Object(pact_core::values::Object::from_hashmap(ns_obj))
                        );
                        return_cek_value(cont, handler, result)
                    }
...
fn namespace_implementation() -> NativeFunction {
    Box::new(|info, builtin, cont, handler, env, args| {
        ...
        if env.call_stack_depth() > 1 {
            return EvalM::pure_value(EvalResult::EvalError(
                pact_core::errors::PactError::PEExecutionError(
                    pact_core::errors::EvalError::InvalidExecutionContext(format!("namespace: Only allowed at top level")),
                    vec![],
                    SpanInfo::empty()
                )
            ));
...
fn static_redeploy_implementation() -> NativeFunction {
    Box::new(|info, _builtin, cont, handler, env, args| {
        ...
        if env.call_stack_depth() > 0 {
            return EvalM::pure_value(EvalResult::EvalError(
                pact_core::errors::PactError::PEExecutionError(
                    pact_core::errors::EvalError::InvalidExecutionContext(format!("static-redeploy: Only allowed at top level")),
                    vec![],
                    SpanInfo::empty()
                )
            ));
                    ...
                    CEKValue::VPactValue(PactValue::String(module_name)) => {
                        ...
                        match read_module_from_storage(&module_name_obj, &*env.pact_db) {
                                ...
                                charge_gas("write", MilliGas(100))
                                    .bind(move |_| {
                                        ...
                                        if let Err(e) = write_module_to_storage(&module_name_obj, &existing_module, &*env.pact_db) {
                                            return EvalM::pure_value(EvalResult::EvalError(
                                                pact_core::errors::PactError::PEExecutionError(
                                                    pact_core::errors::EvalError::InvalidExecutionContext(format!("Failed to redeploy module: {}", e)),
                                                    vec![],
                                                    SpanInfo::empty()
                                                )
                                            ));
                            ...
                            Ok(None) => {
                                ...
                                EvalM::pure_value(EvalResult::EvalError(
                                    pact_core::errors::PactError::PEExecutionError(
                                        pact_core::errors::EvalError::InvalidExecutionContext(format!("Module not found for redeploy: {}", module_name)),
                                        vec![],
                                        SpanInfo::empty()
                                    )
                                ))
                            ...
                            Err(e) => {
                                ...
                                EvalM::pure_value(EvalResult::EvalError(
                                    pact_core::errors::PactError::PEExecutionError(
                                        pact_core::errors::EvalError::InvalidExecutionContext(format!("Error reading module for redeploy: {}", e)),
                                        vec![],
                                        SpanInfo::empty()
                                    )
                                ))
...
) -> Result<Option<StoredModuleInfo>, pact_core::errors::PactErrorI> {
...
fn write_module_to_storage(
    ...
    let module_value = PactValue::Object(pact_core::values::Object::from_hashmap(module_obj));
