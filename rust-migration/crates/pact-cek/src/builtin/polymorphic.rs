...
pub fn register_polymorphic_builtins(builtin_env: &mut BuiltinEnv) -> Result<(), pact_core::errors::PactErrorI> {
                    ...
                    match (args[0].as_pact_value(), args[1].as_pact_value()) {
                        ...
                        (Some(PactValue::Integer(idx)), Some(PactValue::List(list))) => {
                            ...
                            match list.get(index) {
                                ...
                                None => EvalM::throw_error(
                                    pact_core::errors::PactError::PEExecutionError(
                                        pact_core::errors::EvalError::InvalidIndex(index.to_string()),
                                        vec![],
                                        info.clone()
                                    )
                                )
                        ...
                        (Some(PactValue::String(key)), Some(PactValue::Object(obj))) => {
                            match obj.get(key) {
                                ...
                                None => EvalM::throw_error(
                                    pact_core::errors::PactError::PEExecutionError(
                                        pact_core::errors::EvalError::ObjectMissingField(key.clone()),
                                        vec![],
                                        info.clone()
                                    )
                                )
                    ...
                    match (args[0].as_pact_value(), args[1].as_pact_value()) {
                        (Some(PactValue::List(fields)), Some(PactValue::List(objects))) => {
                            if fields.is_empty() || objects.is_empty() {
                                ...
                                let mut indexed: Vec<(usize, pact_core::values::Object)> = 
