...
fn convert_table_schema_to_schema(table_schema: &TableSchema) -> pact_core::errors::Schema {
...
/// Convert pact_core::values::PactValue to pact_core::errors::PactValue for error reporting
fn convert_to_error_pact_value(value: &pact_core::values::PactValue) -> pact_core::errors::PactValue {
  match value {
    pact_core::values::PactValue::String(s) => pact_core::errors::PactValue::PLiteral(s.clone()),
    pact_core::values::PactValue::Integer(i) => pact_core::errors::PactValue::PLiteral(i.to_string()),
    pact_core::values::PactValue::Decimal(d) => pact_core::errors::PactValue::PLiteral(d.to_string()),
    pact_core::values::PactValue::Bool(b) => pact_core::errors::PactValue::PLiteral(b.to_string()),
    pact_core::values::PactValue::Time(t) => pact_core::errors::PactValue::PTime(format!("{:?}", t)),
    pact_core::values::PactValue::List(items) => {
      let converted_items = items.iter().map(convert_to_error_pact_value).collect();
      pact_core::errors::PactValue::PList(converted_items)
    },
    pact_core::values::PactValue::Object(obj) => {
      ...
      pact_core::errors::PactValue::PObject(pact_core::errors::ObjectData { fields: converted_fields })
    },
    _ => pact_core::errors::PactValue::PLiteral(format!("{:?}", value)),
  }
}

/// Check if execution is at top level (not inside a function)
fn enforce_top_level_only(env: &CEKEnv) -> Result<(), pact_core::errors::PactErrorI> {
  // In CEK, we're at top level if the call stack depth is 0
  if env.call_stack_depth() > 0 {
    builtin_error(pact_core::errors::EvalError::InvalidExecutionContext(format!("top-level: Operation only allowed at top level")))
  } else {
    Ok(())
  }
}
  ...
  match guard_type {
    GuardTableType::CreateTable => {
      ...
      if env.module_context.is_none() {
        return builtin_error(pact_core::errors::EvalError::NoModuleContext);
      }
      Ok(())
    }
    GuardTableType::Write => {
      ...
      if !env.has_database_access() {
        return Err(pact_core::errors::PactError::PEExecutionError(
          pact_core::errors::EvalError::CapabilityNotGranted(format!("DATABASE_WRITE for table {}", table_name)),
          vec![],
    ...
    GuardTableType::Read | GuardTableType::Select | GuardTableType::Keys => {
      ...
      if env.flags.allow_read_in_local || env.has_database_access() {
        ...
        Err(pact_core::errors::PactError::PEExecutionError(
          pact_core::errors::EvalError::CapabilityNotGranted(format!("DATABASE_READ for table {}", table_name)),
          vec![],
          pact_core::shared::SpanInfo::empty()
        ))
      }
...
fn check_schema(schema: &TableSchema, obj: &pact_core::values::Object) -> Result<(), pact_core::errors::PactErrorI> {
    ...
    match obj.get(&col_def.name) {
      Some(value) => {
        ...
        match (&col_def.column_type, value) {
          ...
          _ => {
            return builtin_error(pact_core::errors::EvalError::TypeMismatch {
      ...
      None => {
        if !col_def.nullable {
          return builtin_error(pact_core::errors::EvalError::RuntimeError(
...
fn check_partial_schema(schema: &TableSchema, obj: &pact_core::values::Object) -> Result<(), pact_core::errors::PactErrorI> {
...
fn read_implementation() -> NativeFunction {
  Box::new(|_info, _builtin, cont, handler, env, args| {
    ...
    if args.len() != 2 && args.len() != 3 {
      return EvalM::pure_value(EvalResult::EvalError(pact_core::errors::PactError::PEExecutionError(
        pact_core::errors::EvalError::ArgumentCountMismatch { function: "read".to_string(), expected: 2, received: args.len() },
        vec![],
        pact_core::shared::SpanInfo::empty()
      )));
    }
    ...
    charge_gas_with_args("read", &args, MilliGas(30))
      .bind(move |_| {
        match (arg0, arg1) {
                    ...
                    match result {
                      Some(row_data) => {
                        ...
                        charge_gas("read", MilliGas(data_size as u64 + 10))
                          .bind(move |_| {
                            let result_obj = pact_core::values::Object::from_hashmap(row_data.fields);
                      ...
                      None => {
                        ...
                        EvalM::pure_value(EvalResult::EvalError(pact_core::errors::PactError::PEExecutionError(
                          pact_core::errors::EvalError::NoSuchObjectInDb(format!("{}:{}", name, key)),
                          vec![],
                          pact_core::shared::SpanInfo::empty()
                        )))
                      }
          ...
          _ => EvalM::pure_value(EvalResult::EvalError(pact_core::errors::PactError::PEExecutionError(
...
fn insert_implementation() -> NativeFunction {
  Box::new(|_info, _builtin, cont, handler, env, args| {
    if args.len() != 3 {
      return EvalM::pure_value(EvalResult::EvalError(pact_core::errors::PactError::PEExecutionError(
        pact_core::errors::EvalError::ArgumentCountMismatch { function: "insert".to_string(), expected: 3, received: args.len() },
        vec![],
        pact_core::shared::SpanInfo::empty()
      )));
    }
              ...
              Ok(_) => {
                ...
                match check_schema(&schema, &obj) {
                  ...
                  Err(_) => {
                    ...
                    EvalM::pure_value(EvalResult::EvalError(pact_core::errors::PactError::PEExecutionError(
                      pact_core::errors::EvalError::WriteValueDidNotMatchSchema {
                        schema: convert_table_schema_to_schema(&schema),
                        value: pact_core::errors::ObjectData { 
...
fn write_implementation() -> NativeFunction {
  Box::new(|_info, _builtin, cont, handler, env, args| {
    if args.len() != 3 {
      return EvalM::pure_value(EvalResult::EvalError(pact_core::errors::PactError::PEExecutionError(
        pact_core::errors::EvalError::ArgumentCountMismatch { function: "write".to_string(), expected: 3, received: args.len() },
        vec![],
        pact_core::shared::SpanInfo::empty()
      )));
    }
...
fn update_implementation() -> NativeFunction {
  Box::new(|_info, _builtin, cont, handler, env, args| {
    if args.len() != 3 {
      return EvalM::pure_value(EvalResult::EvalError(pact_core::errors::PactError::PEExecutionError(
        pact_core::errors::EvalError::ArgumentCountMismatch { function: "update".to_string(), expected: 3, received: args.len() },
        vec![],
        pact_core::shared::SpanInfo::empty()
      )));
    }
...
fn keys_implementation() -> NativeFunction {
  Box::new(|_info, _builtin, cont, handler, env, args| {
    if args.len() != 1 {
      return EvalM::pure_value(EvalResult::EvalError(pact_core::errors::PactError::PEExecutionError(
        pact_core::errors::EvalError::ArgumentCountMismatch { function: "keys".to_string(), expected: 1, received: args.len() },
        vec![],
        pact_core::shared::SpanInfo::empty()
      )));
    }
...
fn create_table_implementation() -> NativeFunction {
  Box::new(|_info, _builtin, cont, handler, env, args| {
    if args.len() != 1 {
      return EvalM::pure_value(EvalResult::EvalError(pact_core::errors::PactError::PEExecutionError(
        pact_core::errors::EvalError::ArgumentCountMismatch { function: "create-table".to_string(), expected: 1, received: args.len() },
        vec![],
        pact_core::shared::SpanInfo::empty()
      )));
    }
...
fn select_implementation() -> NativeFunction {
  Box::new(|info, _builtin, cont, handler, env, args| {
    if args.len() != 2 {
      return EvalM::pure_value(EvalResult::EvalError(pact_core::errors::PactError::PEExecutionError(
        pact_core::errors::EvalError::ArgumentCountMismatch { function: "select".to_string(), expected: 2, received: args.len() },
        vec![],
        pact_core::shared::SpanInfo::empty()
      )));
    }
            ...
            match guard_table(&env, &name, GuardTableType::Select) {
              Ok(_) => {
                ...
                match filter_fn.can_apply() {
                    ...
                    env.pact_db.select(domain, None).bind(move |rows| {
                      if rows.is_empty() {
                        ...
                        for (key, data) in pact_rows {
                          ...
                          let row_obj = PactValue::Object(pact_core::values::Object::from_hashmap(obj));
                  ...
                  None => EvalM::pure_value(EvalResult::EvalError(pact_core::errors::PactError::PEExecutionError(
                    pact_core::errors::EvalError::TypeMismatch {
...
fn select_with_fields_implementation() -> NativeFunction {
  Box::new(|info, _builtin, cont, handler, env, args| {
    if args.len() != 3 {
      return EvalM::pure_value(EvalResult::EvalError(pact_core::errors::PactError::PEExecutionError(
        pact_core::errors::EvalError::ArgumentCountMismatch { function: "select".to_string(), expected: 3, received: args.len() },
        vec![],
        pact_core::shared::SpanInfo::empty()
      )));
    }
              ...
              Ok(_) => {
                ...
                for field_value in &field_list {
                  match field_value {
                    ...
                    _ => return EvalM::pure_value(EvalResult::EvalError(pact_core::errors::PactError::PEExecutionError(
                      pact_core::errors::EvalError::TypeMismatch {
                  ...
                  Some(filter) => {
                    ...
                    env.pact_db.select(domain, None).bind(move |rows| {
                      ...
                      for (key, data) in rows {
                        ...
                        let mut filtered_obj = pact_core::values::Object::new();
...
fn with_default_read_implementation() -> NativeFunction {
  Box::new(|_info, _builtin, cont, handler, env, args| {
    if args.len() != 4 {
      return EvalM::pure_value(EvalResult::EvalError(pact_core::errors::PactError::PEExecutionError(
        pact_core::errors::EvalError::ArgumentCountMismatch { function: "with-default-read".to_string(), expected: 4, received: args.len() },
        vec![],
        pact_core::shared::SpanInfo::empty()
      )));
    }
                ...
                env.pact_db.read(domain, row_key).bind(move |result| {
                  ...
                  let value = match result {
                    Some(row_data) => {
                      ...
                      charge_gas("read", MilliGas(data_size as u64 + 10))
                        .bind(move |_| {
                          let result_obj = pact_core::values::Object::from_hashmap(fields_clone);
                      ...
                      CEKValue::VPactValue(PactValue::Object(
                        pact_core::values::Object::from_hashmap(row_data.fields)
                      ))
                    }
...
fn with_read_implementation() -> NativeFunction {
  Box::new(|_info, _builtin, cont, handler, env, args| {
    if args.len() != 3 {
      return EvalM::pure_value(EvalResult::EvalError(pact_core::errors::PactError::PEExecutionError(
        pact_core::errors::EvalError::ArgumentCountMismatch { function: "with-read".to_string(), expected: 3, received: args.len() },
        vec![],
        pact_core::shared::SpanInfo::empty()
      )));
    }
...
fn fold_db_implementation() -> NativeFunction {
  Box::new(|info, _builtin, cont, handler, env, args| {
    if args.len() != 3 {
      return EvalM::pure_value(EvalResult::EvalError(pact_core::errors::PactError::PEExecutionError(
        pact_core::errors::EvalError::ArgumentCountMismatch { function: "fold-db".to_string(), expected: 3, received: args.len() },
        vec![],
        pact_core::shared::SpanInfo::empty()
      )));
    }
                      ...
                      if rows.is_empty() {
                        ...
                        let pact_rows: Vec<PactValue> = rows
                          ...
                          .map(|(key, data)| {
                            let mut obj = data.fields;
                            obj.insert("key".to_string(), PactValue::String(key.0));
                            PactValue::Object(pact_core::values::Object::from_hashmap(obj))
                          })
                          .collect();
...
fn describe_keyset_implementation() -> NativeFunction {
  Box::new(|_info, _builtin, cont, handler, env, args| {
    if args.len() != 1 {
      return EvalM::pure_value(EvalResult::EvalError(pact_core::errors::PactError::PEExecutionError(
        pact_core::errors::EvalError::ArgumentCountMismatch { function: "describe-keyset".to_string(), expected: 1, received: args.len() },
        vec![],
        pact_core::shared::SpanInfo::empty()
      )));
    }
                ...
                env.pact_db.read(domain, row_key).bind(move |result| {
                  match result {
                    ...
                    None => {
                      ...
                      EvalM::pure_value(EvalResult::EvalError(pact_core::errors::PactError::PEExecutionError(
                        pact_core::errors::EvalError::NoSuchKeySet(pact_core::errors::KeySetName(keyset_name.clone().into())),
                        vec![],
                        pact_core::shared::SpanInfo::empty()
                      )))
                    }
...
fn describe_module_implementation() -> NativeFunction {
  Box::new(|_info, _builtin, cont, handler, env, args| {
    if args.len() != 1 {
      return EvalM::pure_value(EvalResult::EvalError(pact_core::errors::PactError::PEExecutionError(
        pact_core::errors::EvalError::ArgumentCountMismatch { function: "describe-module".to_string(), expected: 1, received: args.len() },
        vec![],
        pact_core::shared::SpanInfo::empty()
      )));
    }
                ...
                env.pact_db.read(domain, row_key).bind(move |result| {
                  match result {
                    Some(row_data) => {
                      ...
                      charge_gas("read", MilliGas(data_size as u64 + 20))
                        .bind(move |_| {
                          ...
                          let mut module_obj = pact_core::values::Object::new();
                    ...
                    None => {
                      // No such module
                      EvalM::pure_value(EvalResult::EvalError(pact_core::errors::PactError::PEExecutionError(
                        pact_core::errors::EvalError::NoSuchObjectInDb(format!("module:{}", module_name)),
                        vec![],
                        pact_core::shared::SpanInfo::empty()
                      )))
                    }
...
fn describe_table_implementation() -> NativeFunction {
  Box::new(|_info, _builtin, cont, handler, env, args| {
    if args.len() != 1 {
      return EvalM::pure_value(EvalResult::EvalError(pact_core::errors::PactError::PEExecutionError(
        pact_core::errors::EvalError::ArgumentCountMismatch { function: "describe-table".to_string(), expected: 1, received: args.len() },
        vec![],
        pact_core::shared::SpanInfo::empty()
      )));
    }
          ...
          .bind(move |_| {
            match table_arg {
              CEKValue::VTable { name, schema, module_hash: _ } => {
                ...
                let mut result_obj = pact_core::values::Object::new();
...
fn define_keyset_implementation() -> NativeFunction {
  Box::new(|_info, _builtin, cont, handler, env, args| {
    ...
    if args.len() != 1 && args.len() != 2 {
      return EvalM::pure_value(EvalResult::EvalError(pact_core::errors::PactError::PEExecutionError(
        pact_core::errors::EvalError::ArgumentCountMismatch { function: "define-keyset".to_string(), expected: 2, received: args.len() },
        vec![],
        pact_core::shared::SpanInfo::empty()
      )));
    }
...
fn tx_log_implementation() -> NativeFunction {
  Box::new(|_info, _builtin, cont, handler, env, args| {
    if args.len() != 1 {
      return EvalM::pure_value(EvalResult::EvalError(pact_core::errors::PactError::PEExecutionError(
        pact_core::errors::EvalError::ArgumentCountMismatch { function: "tx-log".to_string(), expected: 1, received: args.len() },
        vec![],
        pact_core::shared::SpanInfo::empty()
      )));
    }
...
fn tx_hash_implementation() -> NativeFunction {
  Box::new(|_info, _builtin, cont, handler, _env, args| {
    if args.len() != 0 {
      return EvalM::pure_value(EvalResult::EvalError(pact_core::errors::PactError::PEExecutionError(
        pact_core::errors::EvalError::ArgumentCountMismatch { function: "tx-hash".to_string(), expected: 0, received: args.len() },
        vec![],
        pact_core::shared::SpanInfo::empty()
      )));
    }
...
fn read_with_fields_implementation() -> NativeFunction {
  Box::new(|_info, _builtin, cont, handler, env, args| {
    if args.len() != 3 {
      return EvalM::pure_value(EvalResult::EvalError(pact_core::errors::PactError::PEExecutionError(
        pact_core::errors::EvalError::ArgumentCountMismatch { function: "read".to_string(), expected: 3, received: args.len() },
        vec![],
        pact_core::shared::SpanInfo::empty()
      )));
    }
...
pub fn register_database_builtins(builtin_env: &mut BuiltinEnv) -> Result<(), pact_core::errors::PactErrorI> {
