...
use pact_core::values::{PactValue, CapToken, Guard};
use pact_core::gas::MilliGas;

/// Register all capability builtin functions
pub fn register_capability_builtins(builtin_env: &mut BuiltinEnv) -> Result<(), pact_core::errors::PactErrorI> {
...
fn require_capability(
    ...
    match args.as_slice() {
            ...
            charge_gas_with_args("require-capability", &[], gas_cost).bind(move |_| {
                ...
                if cap_in_stack {
                    ...
                    EvalM::throw_error(pact_core::errors::PactError::PEExecutionError(
                        pact_core::errors::EvalError::CapabilityNotGranted(cap_name),
                        vec![],
        ...
        _ => {
            ...
            EvalM::throw_error(pact_core::errors::PactError::PEExecutionError(
                pact_core::errors::EvalError::InvalidArgument(format!("require-capability: Expected single capability token argument")),
                vec![],
                pact_core::shared::SpanInfo::empty()
            ))
        }
...
fn install_capability(
    ...
    match args.as_slice() {
            ...
            charge_gas_with_args("install-capability", &[], MilliGas(20)).bind(move |_| {
                ...
                if within_defcap {
                    return EvalM::throw_error(pact_core::errors::PactError::PEExecutionError(
                        pact_core::errors::EvalError::InvalidExecutionContext(format!("install-capability: Cannot install capability within defcap")),
                        vec![],
                        pact_core::shared::SpanInfo::empty()
                    ));
                }
                ...
                if cap_in_stack {
                    return EvalM::throw_error(pact_core::errors::PactError::PEExecutionError(
                        pact_core::errors::EvalError::InvalidArgument(format!("install-capability: Capability {} already installed", cap_name)),
                        vec![],
                        info.clone()
                    ));
                }
        ...
        _ => {
            // Argument error - expected exactly one capability token
            EvalM::throw_error(pact_core::errors::PactError::PEExecutionError(
                pact_core::errors::EvalError::InvalidArgument(format!("install-capability: Expected single capability token argument")),
                vec![],
                pact_core::shared::SpanInfo::empty()
            ))
        }
    ...
    match args.as_slice() {
        [CEKValue::VPactValue(PactValue::CapToken(cap_token))] => {
            ...
            charge_gas_with_args("compose-capability", &[], MilliGas(15)).bind(move |_| {
                ...
                if !within_defcap {
                    return EvalM::throw_error(pact_core::errors::PactError::PEExecutionError(
                        pact_core::errors::EvalError::InvalidExecutionContext(format!("compose-capability: compose-capability must be called within a defcap")),
                        vec![],
                        pact_core::shared::SpanInfo::empty()
                    ));
                }
        ...
        _ => {
            // Argument error - expected exactly one capability token
            EvalM::throw_error(pact_core::errors::PactError::PEExecutionError(
                pact_core::errors::EvalError::InvalidArgument(format!("compose-capability: Expected single capability token argument")),
                vec![],
                pact_core::shared::SpanInfo::empty()
            ))
        }
...
fn emit_event(
    ...
    match args.as_slice() {
            ...
            charge_gas_with_args("emit-event", &[], MilliGas(10)).bind(move |_| {
                ...
                if let Some(module_name) = module_name {
                    ...
                    if !cap_name.starts_with(&format!("{}.", module_name)) {
                        return EvalM::throw_error(pact_core::errors::PactError::PEExecutionError(
                            pact_core::errors::EvalError::InvalidExecutionContext(format!("emit-event: Capability {} must be defined in current module", cap_name)),
                            vec![],
                            pact_core::shared::SpanInfo::empty()
                        ));
                    }
        ...
        _ => {
            // Argument error - expected exactly one capability token
            EvalM::throw_error(pact_core::errors::PactError::PEExecutionError(
                pact_core::errors::EvalError::InvalidArgument(format!("emit-event: Expected single capability token argument")),
                vec![],
                pact_core::shared::SpanInfo::empty()
            ))
        }
...
fn create_capability_guard(
    ...
    match args.as_slice() {
        ...
        _ => {
            // Argument error - expected exactly one capability token
            EvalM::throw_error(pact_core::errors::PactError::PEExecutionError(
                pact_core::errors::EvalError::InvalidArgument(format!("create-capability-guard: Expected single capability token argument")),
                vec![],
                pact_core::shared::SpanInfo::empty()
            ))
        }
...
fn create_capability_pact_guard(
    ...
    match args.as_slice() {
        [CEKValue::VPactValue(PactValue::CapToken(cap_token))] => {
            ...
            charge_gas_with_args("create-capability-pact-guard", &[], MilliGas(5)).bind(move |_| {
                ...
                let pact_id = match pact_id {
                    ...
                    None => {
                        return EvalM::throw_error(pact_core::errors::PactError::PEExecutionError(
                            pact_core::errors::EvalError::InvalidExecutionContext(format!("create-capability-pact-guard: No pact in scope")),
                            vec![],
                            pact_core::shared::SpanInfo::empty()
                        ));
                    }
        ...
        _ => {
            // Argument error - expected exactly one capability token
            EvalM::throw_error(pact_core::errors::PactError::PEExecutionError(
                pact_core::errors::EvalError::InvalidArgument(format!("create-capability-pact-guard: Expected single capability token argument")),
                vec![],
                pact_core::shared::SpanInfo::empty()
            ))
        }
