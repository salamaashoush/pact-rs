...
use pact_core::values::{Decimal, PactValue};
...
pub fn register_arithmetic_builtins(builtin_env: &mut BuiltinEnv) -> Result<(), pact_core::errors::PactErrorI> {
    ...
    charge_gas_with_args("/", &args, MilliGas(4))
      .bind(move |_| {
        ...
        match (args[0].as_pact_value(), args[1].as_pact_value()) {
            ...
            (Some(PactValue::Decimal(a)), Some(PactValue::Decimal(b))) => {
              if *b == pact_core::values::Decimal::from(0) {
