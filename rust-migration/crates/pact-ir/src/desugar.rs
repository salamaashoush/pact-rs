...
pub fn desugar_interface(
    interface: pact_syntax::parser::ParsedInterface<ParsedSpanInfo>,
    ctx: &mut DesugarContext,
...
fn desugar_if_def(
    def: pact_syntax::parser::ParsedIfDef<ParsedSpanInfo>,
    ctx: &mut DesugarContext,
) -> Result<DesugarOutput<IfDef<Name, Type, CoreBuiltin, SpanInfo>>, PactError<SpanInfo>> {
    use pact_syntax::parser::ParsedIfDef;
    ...
    let body = if defun.body.len() == 1 {
        ...
        while let Some(term) = terms.pop() {
            result = Term::Sequence {
                ...
                info: SpanInfo::empty(), // TODO: proper span info
            };
        }
        result
    };
...
fn desugar_span_info(info: ParsedSpanInfo) -> SpanInfo {
    // ParsedSpanInfo and SpanInfo are the same type from pact_core
...
fn desugar_dcap_meta_with_validation(
    meta: pact_syntax::parser::DCapMeta,
    args: &[pact_syntax::parser::MArg<ParsedSpanInfo>],
    info: ParsedSpanInfo,
) -> Result<Option<DCapMeta>, PactError<SpanInfo>> {
    use pact_syntax::parser::DCapMeta as ParsedDCapMeta;
...
fn desugar_pact_step(
    step: pact_syntax::parser::PactStep<ParsedSpanInfo>,
    ctx: &mut DesugarContext,
) -> Result<DesugarOutput<PactStep<Name, Type, CoreBuiltin, SpanInfo>>, PactError<SpanInfo>> {
    use pact_syntax::parser::PactStep as AstPactStep;
...
fn desugar_dcap_meta(meta: pact_syntax::parser::DCapMeta) -> Result<DCapMeta, PactError<SpanInfo>> {
    use pact_syntax::parser::DCapMeta as AstDCapMeta;
