...
use pact_syntax::{Parser, ParsedTopLevel, SpanInfo};
...
pub struct CompileResult {
    ...
    pub top_level: TopLevel<pact_ir::Name, pact_ir::Type, pact_ir::CoreBuiltin, pact_syntax::SpanInfo>,
...
pub fn compile_parsed_top_level(
    parsed_toplevel: ParsedTopLevel<pact_syntax::SpanInfo>,
    ctx: &mut CompilationContext,
) -> Result<TopLevel<pact_ir::Name, pact_ir::Type, pact_ir::CoreBuiltin, pact_syntax::SpanInfo>, PactError<SpanInfo>> {
    ...
    let hashed = hash_top_level(const_evaled, &mut ctx.hash_ctx)
        .map_err(|e| PactError::PEExecutionError(
            pact_core::errors::EvalError::RuntimeError(format!("Failed to hash module: {}", e)),
            vec![],
            pact_syntax::SpanInfo::empty()
        ))?;
...
fn parse_source_code(source_code: &str) -> Result<ParsedTopLevel<pact_syntax::SpanInfo>, PactError<SpanInfo>> {
    // Create parser (which handles lexing internally)
    let mut parser = Parser::new(source_code)
        .map_err(|e| PactError::PEExecutionError(
            pact_core::errors::EvalError::UserError(format!("Parser creation failed: {:?}", e)),
            vec![],
            SpanInfo::empty()
        ))?;
    ...
    let program = parser.parse_program()
        .map_err(|e| PactError::PEExecutionError(
            pact_core::errors::EvalError::UserError(format!("Parsing failed: {:?}", e)),
            vec![],
            SpanInfo::empty()
        ))?;
    ...
    match program.len() {
        0 => Err(PactError::PEExecutionError(
            pact_core::errors::EvalError::UserError("No top-level items found in source code".to_string()),
            vec![],
            SpanInfo::empty()
        )),
        ...
        _ => {
            ...
            for item in program {
                match item {
                    pact_syntax::ParsedTopLevel::TLTerm(expr) => expressions.push(expr),
            ...
            if expressions.len() > 1 {
                ...
                Ok(pact_syntax::ParsedTopLevel::TLTerm(block_expr))
            } else if expressions.len() == 1 {
                Ok(pact_syntax::ParsedTopLevel::TLTerm(expressions.into_iter().next().unwrap()))
            } else {
...
fn get_combined_span(expressions: &[pact_syntax::ParsedExpr<pact_syntax::SpanInfo>]) -> pact_syntax::SpanInfo {
    let first_span = expressions.first().unwrap().span();
    let last_span = expressions.last().unwrap().span();
    pact_syntax::SpanInfo {
...
fn create_block_expression(
    expressions: Vec<pact_syntax::ParsedExpr<pact_syntax::SpanInfo>>,
    span: pact_syntax::SpanInfo,
) -> pact_syntax::ParsedExpr<pact_syntax::SpanInfo> {
    ...
    pact_syntax::ParsedExpr::Let {
        form: pact_syntax::parser::LetForm::LFLetNormal,
        ...
        match compile_top_level(source_code, ctx) {
            ...
            Err(e) => {
                return Err(PactError::PEExecutionError(
                    pact_core::errors::EvalError::UserError(format!("Compilation failed for {}: {}", filename, e)),
                    vec![],
                    SpanInfo::empty()
                ));
            }
