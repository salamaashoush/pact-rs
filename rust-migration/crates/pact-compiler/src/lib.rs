...
// Re-export from consolidated pact-modules
pub use pact_modules::ModuleStorageManager;
...
pub fn compile_and_evaluate(source: &str) -> Result<(CompileResult, pact_core::values::PactValue), pact_core::errors::PactError<pact_syntax::SpanInfo>> {
...
) -> Result<(CompileResult, pact_core::values::PactValue), pact_core::errors::PactError<pact_syntax::SpanInfo>> {
    // First, compile the source
    let compile_result = compile_pact_source(source)?;
