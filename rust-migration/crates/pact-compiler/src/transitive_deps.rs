...
/// Uses BTreeSet for deterministic ordering matching Haskell's S.minView
...
impl WorkingSet {
    ...
    pub fn pop(&mut self) -> Option<FullyQualifiedName> {
        // Use pop_first for deterministic minimum element (matching Haskell's S.minView)
        self.0.pop_first()
    }
...
impl TransitiveClosureState {
    ...
    fn charge_set_search(&mut self, size: usize) -> Result<(), TransitiveClosureError> {
        if size > 0 {
            ...
            // Match Haskell: uses log base 10, not natural log
...
fn restrict_keys_cost(map_size: usize, set_size: usize) -> u64 {
    ...
    // Match Haskell: uses log base 10, not natural log
    ((m * ((n / m) + 1.0).log10()).ceil() as u64) * COST_PER_FQN_COMPARISON
}
    ...
    match state.compute_transitive_deps() {
        ...
        Err(TransitiveClosureError::UnboundFreeVariable(fqn)) => {
            Err(PactError::PEExecutionError(
                pact_core::errors::EvalError::UnboundVariable(format!("{}", fqn)),
                vec![],
                SpanInfo::empty()
            ))
        }
        Err(TransitiveClosureError::GasLimitExceeded { limit, consumed }) => {
            Err(PactError::PEExecutionError(
                pact_core::errors::EvalError::GasExceeded { 
                    limit: pact_core::errors::GasLimit(limit.0.0 / 1000), // Convert milligas to gas
                    used: pact_core::errors::Gas(consumed.0 / 1000) // Convert milligas to gas
                },
                vec![],
                SpanInfo::empty()
            ))
...
mod tests {
    ...
    fn test_working_set() {
        ...
        assert_eq!(ws.len(), 2);
        // BTreeSet guarantees deterministic order - "bar" comes before "foo"
        let popped1 = ws.pop().unwrap();
        let popped2 = ws.pop().unwrap();
        assert_eq!(popped1, fqn2); // "bar" should be popped first
        assert_eq!(popped2, fqn1); // "foo" should be popped second
