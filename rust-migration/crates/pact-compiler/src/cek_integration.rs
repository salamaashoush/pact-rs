...
use pact_modules::ModuleStorageManager;
use pact_db::MockDb;
...
impl std::fmt::Debug for CekDbAdapter {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("CekDbAdapter")
            .field("db", &"<dyn PactDb>")
            .finish()
    }
}
...
impl pact_cek::PactDb for CekDbAdapter {
    fn read(&self, domain: pact_cek::Domain, key: pact_cek::RowKey) -> pact_cek::EvalM<Option<pact_cek::RowData>> {
        ...
        EvalM::try_from_io(move || {
            db.read_row(&pact_db::TableName::new(table_name, ModuleName::simple("user")), &db_key)
                .map(|opt| opt.map(|row_data| {
                    // Convert pact_db::RowData to pact_cek::RowData
                    let mut cek_row = std::collections::HashMap::new();
                    for (k, v) in &row_data.data {
                        cek_row.insert(k.clone(), v.clone());
                    }
                    pact_cek::RowData { fields: cek_row }
                }))
    ...
    fn write_with_type(&self, domain: pact_cek::Domain, key: pact_cek::RowKey, data: pact_cek::RowData, write_type: pact_cek::WriteType) -> pact_cek::EvalM<()> {
        ...
        EvalM::try_from_io(move || {
            db.write_row(db_write_type, &pact_db::TableName::new(table_name, ModuleName::simple("user")), &db_key, &db_row_data)
    ...
    fn keys(&self, domain: pact_cek::Domain) -> pact_cek::EvalM<Vec<pact_cek::RowKey>> {
        use pact_cek::EvalM;
        let table_name = match &domain {
            pact_cek::Domain::User(t) => pact_db::TableName::new(t.name.clone(), ModuleName::simple("user")),
            _ => pact_db::TableName::new("".to_string(), ModuleName::simple("user")),
        };
        let db = self.db.clone();
        
        EvalM::try_from_io(move || {
            db.table_keys(&table_name)
    ...
    fn select(&self, domain: pact_cek::Domain, _filter: Option<pact_cek::EvalM<bool>>) -> pact_cek::EvalM<Vec<(pact_cek::RowKey, pact_cek::RowData)>> {
        ...
        EvalM::try_from_io(move || {
            db.select_rows(&table_name, &pact_db::SelectCriteria::new())
                .map(|rows| {
                    rows.into_iter().map(|(k, v)| {
                        let mut cek_row = std::collections::HashMap::new();
                        for (field_k, field_v) in &v.data {
                            cek_row.insert(field_k.clone(), field_v.clone());
                        }
                        (pact_cek::RowKey(k.to_string()), pact_cek::RowData { fields: cek_row })
                    }).collect()
                })
    ...
    fn create_table(&self, table_name: String, _schema: pact_cek::TableSchema) -> pact_cek::EvalM<()> {
        use pact_cek::EvalM;
        let db = self.db.clone();
        EvalM::try_from_io(move || {
            // Create empty schema for table creation
            let empty_schema = pact_schema::Schema {
                name: pact_schema::QualifiedName::new(ModuleName::simple("user"), "temp".to_string()),
                fields: std::collections::HashMap::new(),
            };
            db.create_user_table(&pact_db::TableName::new(table_name, ModuleName::simple("user")), &empty_schema)
    ...
    fn describe_table(&self, table: pact_cek::TableName) -> pact_cek::EvalM<Option<pact_schema::Schema>> {
        ...
        EvalM::try_from_io(move || {
            db.get_table_schema(&pact_db::TableName::new(table_name, ModuleName::simple("user")))
    ...
    fn table_exists(&self, table: pact_cek::TableName) -> pact_cek::EvalM<bool> {
        ...
        let table_name = table.name.clone();
        EvalM::try_from_io(move || {
            db.table_exists(&pact_db::TableName::new(table_name, ModuleName::simple("user")))
...
pub fn evaluate_with_cek(
    top_level: &TopLevel<pact_ir::Name, pact_ir::Type, pact_ir::CoreBuiltin, pact_syntax::SpanInfo>,
...
impl pact_cek::PactDb for SimpleMockDb {
    ...
    fn begin_tx(&self, _mode: pact_cek::ExecutionMode) -> pact_cek::EvalM<Option<pact_cek::TxId>> {
        pact_cek::EvalM::pure_value(None)
    }
    ...
    fn describe_table(&self, _table: pact_cek::TableName) -> pact_cek::EvalM<Option<pact_schema::Schema>> {
        pact_cek::EvalM::pure_value(None)
    }

    fn table_exists(&self, _table: pact_cek::TableName) -> pact_cek::EvalM<bool> {
        pact_cek::EvalM::pure_value(false)
    }
