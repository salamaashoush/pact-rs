...
pub struct CEKEnv {
    ...
    pub chain_id: Option<pact_shared_types::ChainId>,
}
...
pub struct ModuleContext {
    ...
    pub hash: pact_names::ModuleHash,
...
pub trait PactDb: std::fmt::Debug + Send + Sync {
    ...
    fn write_row(&self, table: &str, key: &str, value: PactValue) -> Result<(), pact_errors::PactErrorI> {
        // Simplified sync version for compatibility
        Ok(())
    }

    fn select_rows(&self, table: &str, filter: Option<&str>) -> Result<Vec<(String, PactValue)>, pact_errors::PactErrorI> {
        // Simplified sync version for compatibility
        Ok(vec![])
    }
}
...
impl BuiltinEnv {
    ...
    pub fn lookup(&self, builtin: CoreBuiltin, info: SpanInfo) -> Result<NativeFn, pact_errors::PactErrorI> {
        self.functions.get(&builtin)
            ...
            .ok_or_else(|| pact_errors::PactError::PEExecutionError(
                pact_errors::EvalError::RuntimeError(format!("Unknown builtin: {:?}", builtin)),
                vec![],
                info
            ))
    }
...
impl GasState {
    ...
    /// Charge gas
    pub fn charge_gas(&self, cost: MilliGas) -> Result<GasState, pact_errors::PactErrorI> {
        let new_used = self.gas_used + cost.value();
        if new_used > self.gas_limit {
            Err(pact_errors::PactError::PEExecutionError(
                pact_errors::EvalError::RuntimeError(format!(
