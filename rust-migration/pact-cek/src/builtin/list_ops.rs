...
pub fn register_list_builtins(builtin_env: &mut BuiltinEnv) -> Result<(), pact_errors::PactErrorI> {
                        ...
                        (Some(PactValue::Integer(start)), Some(PactValue::Integer(end)), Some(PactValue::Integer(step))) => {
                            ...
                            if step_i64 == 0 {
                                return EvalM::pure_value(EvalResult::EvalError(
                                    pact_errors::PactError::PEExecutionError(
                                        pact_errors::EvalError::InvalidArgument(format!("enumerate: step cannot be zero")),
                                        vec![],
                                        info.clone()
                                    )
                                ));
