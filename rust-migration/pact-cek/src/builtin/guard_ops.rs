use pact_values::{PactValue, Guard, Keyset};
...
pub fn register_guard_builtins(builtin_env: &mut BuiltinEnv) -> Result<(), pact_errors::PactErrorI> {
...
fn enforce_guard_value(guard: &Guard, env: &CEKEnv) -> Result<(), pact_errors::PactErrorI> {
    match guard {
        Guard::KeySet { name, keys, pred } => {
            if keys.is_empty() {
                ...
                let keyset = Keyset::new(keys.clone(), pact_values::KeySetPredicate::parse(pred));
        ...
        Guard::Capability { name, args } => {
            ...
            if env.has_capability(name, args) {
                ...
                builtin_error(pact_errors::EvalError::CapabilityNotGranted(name.clone()))
            }
        }
        Guard::Module { name, governance: _ } => {
            ...
            if env.has_module_admin(name) {
                ...
                Err(pact_errors::PactError::PEExecutionError(
                    pact_errors::EvalError::GuardFailure(format!("module admin required for {}", name)),
                    vec![],
...
fn enforce_keyset_value(keyset: &Keyset, env: &CEKEnv) -> Result<(), pact_errors::PactErrorI> {
...
fn enforce_keyset_by_name(keyset_name: &str, env: &CEKEnv) -> Result<(), pact_errors::PactErrorI> {
...
fn create_principal_for_guard(guard: &Guard) -> String {
    match guard {
        ...
        Guard::Capability { name, args } => {
            ...
            let cap_token = pact_values::CapToken {
                name: name.clone(),
                args: args.clone(),
            };
...
fn compute_capability_hash(capability: &pact_values::CapToken) -> String {
    // In practice, this would compute a cryptographic hash
                            ...
                            None => {
                                EvalM::pure_value(EvalResult::EvalError(
                                    pact_errors::PactError::PEExecutionError(
                                        pact_errors::EvalError::InvalidArgument(format!("typeof-principal: Invalid principal: {}", principal_str)),
                                        vec![],
                                        info.clone()
                                    )
                                ))
                ...
                match arg0 {
                    CEKValue::VPactValue(PactValue::String(keyset_name)) => {
                        ...
                        let keyset = Keyset::new(
                            ...
                            pact_values::KeySetPredicate::KeysAll
                        );
                ...
                match arg0 {
                    CEKValue::VPactValue(PactValue::String(keyset_name)) => {
                        ...
                        let result = CEKValue::VPactValue(PactValue::Object(
                            pact_values::Object::from_hashmap(keyset_info)
                        ));
                        return_cek_value(cont, handler, result)
                    }
