...
use pact_shared_types::{SpanInfo, ChainId};
use pact_values::{PactValue, Object};
use pact_names::ModuleHash;
...
pub fn register_defpact_builtins(builtin_env: &mut BuiltinEnv) -> Result<(), pact_errors::PactErrorI> {
...
fn yield_implementation(
    ...
    if args.len() != 1 {
        return EvalM::throw_error(pact_errors::PactError::PEExecutionError(
            pact_errors::EvalError::ArgumentCountMismatch { function: "yield".to_string(), expected: 1, received: args.len() },
            vec![],
    ...
    match &arg0 {
                ...
                match pact_step {
                    ...
                    None => {
                        EvalM::throw_error(pact_errors::PactError::PEExecutionError(
                            pact_errors::EvalError::InvalidExecutionContext(format!("yield: yield called outside defpact execution")),
                            vec![],
                            pact_shared_types::SpanInfo::empty()
                        ))
                    }
        ...
        _ => {
            EvalM::throw_error(pact_errors::PactError::PEExecutionError(
                pact_errors::EvalError::InvalidArgument(format!("yield: Expected object argument")),
                vec![],
                pact_shared_types::SpanInfo::empty()
            ))
        }
...
fn yield_to_chain_implementation(
    ...
    if args.len() != 2 {
        return EvalM::throw_error(pact_errors::PactError::PEExecutionError(
            pact_errors::EvalError::ArgumentCountMismatch { function: "yield".to_string(), expected: 2, received: args.len() },
            vec![],
            pact_shared_types::SpanInfo::empty()
        ));
    }
    ...
    match (&arg0, &arg1) {
                ...
                match pact_step {
                    Some(step_state) => {
                        ...
                        if step_state.rollback {
                            return EvalM::throw_error(pact_errors::PactError::PEExecutionError(
                                pact_errors::EvalError::InvalidExecutionContext(format!("yield: Cross-chain yield not allowed in step with rollback")),
                                vec![],
                                pact_shared_types::SpanInfo::empty()
                            ));
                        }
                        ...
                        let module_hash = match module_context {
                            ...
                            None => {
                                return EvalM::throw_error(pact_errors::PactError::PEExecutionError(
                                    pact_errors::EvalError::NoModuleContext,
                                    vec![],
                                    pact_shared_types::SpanInfo::empty()
                                ));
                            }
        ...
        _ => {
            EvalM::throw_error(pact_errors::PactError::PEExecutionError(
                pact_errors::EvalError::InvalidArgument(format!("yield: Expected (object, chain-id) arguments")),
                vec![],
                pact_shared_types::SpanInfo::empty()
            ))
        }
...
fn resume_implementation(
    ...
    if args.len() != 1 {
        return EvalM::throw_error(pact_errors::PactError::PEExecutionError(
            pact_errors::EvalError::ArgumentCountMismatch { function: "resume".to_string(), expected: 1, received: args.len() },
            vec![],
            pact_shared_types::SpanInfo::empty()
        ));
    }
    ...
    match &closure {
                ...
                match pact_step {
                    Some(step_state) => {
                        ...
                        match step_state.step_result {
                            ...
                            None => {
                                EvalM::throw_error(pact_errors::PactError::PEExecutionError(
                                    pact_errors::EvalError::InvalidExecutionContext(format!("resume: No yield data available in defpact step")),
                                    vec![],
                                    pact_shared_types::SpanInfo::empty()
                                ))
                            }
                    ...
                    None => {
                        EvalM::throw_error(pact_errors::PactError::PEExecutionError(
                            pact_errors::EvalError::InvalidExecutionContext(format!("resume: resume called outside defpact execution")),
                            vec![],
                            pact_shared_types::SpanInfo::empty()
                        ))
                    }
        ...
        _ => {
            EvalM::throw_error(pact_errors::PactError::PEExecutionError(
                pact_errors::EvalError::InvalidArgument(format!("resume: Expected closure argument")),
                vec![],
                pact_shared_types::SpanInfo::empty()
            ))
        }
...
fn pact_id_implementation(
    ...
    if !args.is_empty() {
        return EvalM::throw_error(pact_errors::PactError::PEExecutionError(
            pact_errors::EvalError::ArgumentCountMismatch { function: "pact-id".to_string(), expected: 0, received: args.len() },
            vec![],
            pact_shared_types::SpanInfo::empty()
        ));
    }
    ...
    charge_gas_with_args("pact-id", &[], MilliGas(2)).bind(move |_| {
        match env.defpact_step {
            ...
            None => {
                EvalM::throw_error(pact_errors::PactError::PEExecutionError(
                    pact_errors::EvalError::InvalidExecutionContext(format!("pact-id: pact-id called outside defpact execution")),
                    vec![],
                    pact_shared_types::SpanInfo::empty()
                ))
            }
