...
pub fn register_boolean_builtins(builtin_env: &mut BuiltinEnv) -> Result<(), pact_errors::PactErrorI> {
        ...
        match args.as_slice() {
            ...
            _ => {
                ...
                EvalM::throw_error(pact_errors::PactError::PEExecutionError(
                    pact_errors::EvalError::InvalidArgument(format!("not: Expected single boolean argument, got {} arguments", args.len())),
                    vec![],
                    info.clone()
                ))
            }
    ...
    charge_gas_with_args("not?", &args, MilliGas(2)).bind(move |_| {
        match args.as_slice() {
            ...
            _ => {
                ...
                EvalM::throw_error(pact_errors::PactError::PEExecutionError(
                    pact_errors::EvalError::InvalidArgument(format!("not?: Expected closure and value arguments, got {} arguments", args.len())),
                    vec![],
                    info.clone()
                ))
            }
    ...
    charge_gas_with_args("and?", &args, MilliGas(3)).bind(move |_| {
        match args.as_slice() {
            ...
            _ => {
                ...
                EvalM::throw_error(pact_errors::PactError::PEExecutionError(
                    pact_errors::EvalError::InvalidArgument(format!("and?: Expected two closures and value arguments, got {} arguments", args.len())),
                    vec![],
                    info.clone()
                ))
            }
    ...
    charge_gas_with_args("or?", &args, MilliGas(3)).bind(move |_| {
        match args.as_slice() {
            ...
            _ => {
                // Argument error - expected two closures and a value
                EvalM::throw_error(pact_errors::PactError::PEExecutionError(
                    pact_errors::EvalError::InvalidArgument(format!("or?: Expected two closures and value arguments, got {} arguments", args.len())),
                    vec![],
                    info.clone()
                ))
            }
