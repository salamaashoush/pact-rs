...
use pact_errors::{PactError, EvalError, PactErrorI};
...
pub struct GrantedCapability {
    ...
    pub args: Vec<pact_values::PactValue>,
...
pub struct DefPactExecution {
    ...
    pub total_steps: u32,
    /// Step results
    pub step_results: Vec<pact_values::PactValue>,
    /// Continuation chain
...
pub struct DefPactHistoryEntry {
    ...
    pub step: u32,
    /// Step result
    pub result: pact_values::PactValue,
    /// Execution timestamp
    ...
    match operation {
        ...
        "map" | "filter" | "fold" => {
            if let Some(list_arg) = args.get(1) {
                if let Some(pact_values::PactValue::List(list)) = list_arg.as_pact_value() {
        ...
        "concat" => {
            let total_length = args.iter()
                ...
                .filter_map(|pv| match pv {
                    pact_values::PactValue::String(s) => Some(s.len()),
                    _ => None,
                })
                .sum::<usize>() as u64;
        ...
        "length" | "reverse" => {
            if let Some(arg) = args.get(0) {
                match arg.as_pact_value() {
                    Some(pact_values::PactValue::String(s)) => {
                        MilliGas(base_cost.0 + (s.len() as u64 / 10)) // 1 gas per 10 chars
                    }
                    Some(pact_values::PactValue::List(list)) => {
...
fn estimate_pact_value_size(value: &pact_values::PactValue) -> u64 {
    match value {
        pact_values::PactValue::String(s) => s.len() as u64 / 4, // 1 gas per 4 chars
        pact_values::PactValue::Integer(_) => 1,
        pact_values::PactValue::Decimal(_) => 2,
        pact_values::PactValue::Bool(_) => 1,
