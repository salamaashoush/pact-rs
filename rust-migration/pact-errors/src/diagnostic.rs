...
use crate::{PactError, StackFrame};
...
fn write_source_snippet(
    ...
    let (start_line, start_col) = byte_offset_to_line_col(source_info.source, span.start);
    let (end_line, end_col) = byte_offset_to_line_col(source_info.source, span.end);
...
fn get_error_suggestion(error: &PactError<SpanInfo>) -> Option<String> {
    use crate::{EvalError, ParseError, LexerError};
